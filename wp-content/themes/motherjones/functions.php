...
class MJ
{
    ...
    private function require_files() 
    {
        ...
        if (! class_exists('Navis_Media_Credit') ) {
            include_once dirname(__FILE__) . '/vendor/largo/media-credit.php';
        }
    }
    ...
    private function register_media_sizes() 
    {
        ...
        add_filter(
            'pre_option_thumbnail_size_w', function () {
                return 208;
            }
        );
        add_filter(
            'pre_option_thumbnail_size_h', function () {
                return 117;
            }
        );
        add_filter('pre_option_thumbnail_crop', '__return_true');
        add_filter(
            'pre_option_medium_size_w', function () {
                return MEDIUM_WIDTH;
            }
        );
        add_filter(
            'pre_option_medium_size_h', function () {
                return MEDIUM_HEIGHT;
            }
        );
        add_filter(
            'pre_option_large_size_w', function () {
                return LARGE_WIDTH;
            }
        );
        add_filter(
            'pre_option_large_size_h', function () {
                return LARGE_HEIGHT;
            }
        );
        add_filter('pre_option_embed_autourls', '__return_true');
        add_filter(
            'pre_option_embed_size_w', function () {
                return LARGE_WIDTH;
            }
        );
        add_filter(
            'pre_option_embed_size_h', function () {
                return LARGE_HEIGHT;
            }
        );
